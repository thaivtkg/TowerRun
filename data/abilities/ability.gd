...
func is_ready(current_energy: float = 0.0) -> bool:
    ...
    # Bắt buộc kiểm tra Cooldown trước cho tất cả các loại Kỹ năng (kể cả PASSIVE)
    if current_cooldown > 0.0:
        return false

    # Passive không tiêu tốn Năng lượng, chỉ cần hết Cooldown là sẵn sàng
    if data.ability_type == AbilityData.AbilityType.PASSIVE:
        return true

    return current_energy >= data.energy_cost
